...
@section('content')
    <div class="d-flex justify-content-center">
        <div class="col-md-7 col-lg-8">
            ...
            <form class="needs-validation" novalidate="" action="{{ route('users.update', ['user' => $user->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-sm-6">
                        <label for="firstName" class="form-label">First name</label>
                        <input type="text" name="first_name" value="{{ old('first_name', $user->first_name) }}" class="form-control @error('first_name') is-invalid @enderror" id="firstName" placeholder="" required>
                    ...
                    <div class="col-sm-6">
                        ...
                        <input type="text" name="last_name" value="{{ old('last_name', $user->last_name) }}" class="form-control @error('last_name') is-invalid @enderror" id="lastName" placeholder="" required>
                        <div class="invalid-feedback">
                            @error('last_name')
                            {{ $message }}
                            @enderror
                    ...
                    <div class="col-12">
                        ...
                        <div class="input-group has-validation">
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="form-control @error('email') is-invalid @enderror" id="username" placeholder="example@gmail.com" required>
                            <div class="invalid-feedback">
                                @error('email')
                                {{ $message }}
                                @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="department" class="form-select @error('department') is-invalid @enderror" id="department" required>
                            ...
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}" {{ ($department->id == old('department', $user->department_id)) ? 'selected' : '' }}>{{ $department->name }}</option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">
                            @error('department')
                    ...
                    <?php
                        ...
                        foreach ($user->user_roles as $ur) {
                            array_push($roles_id_array, $ur->role_id);
                        }
                    ?>
                    @for ($i = 0; $i < 3; $i++)
                        <div class="col-md-4">
                            ...
                            <select name="role[]" class="form-select @error('role.'.$i) is-invalid @enderror" id="role" required>
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" {{ ($role->id == old('role.'.$i, ($roles_id_array[$i] ?? null))) ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback">
                                @error('role.'.$i)
                    ...
                    <div class="col-12">
                        <div class="mb-2">
                            <img class="bg-white" src="{{ asset($user->GetAvatar()) }}" alt="male_profile.png" width="150px">
                        </div>
                        <label for="profile_image" class="form-label">
                            Choose a profile image to change
                        </label>
                ...
                <div class="d-flex justify-content-around mt-5 row">
                    <a href="{{ route('users.show', ['user' => $user->id]) }}" class="text-white text-decoration-none btn btn-danger btn-lg w-25"> Back</a>
